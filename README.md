...
src/rcr/Surfing.java                 True Surfing 走位（两波精确预测 + KNN 危险密度 + shadows/flattener）
src/rcr/KnnGun.java                  KNN(DC) 双枪（通用 + anti-surfer，虚拟枪框架选枪）
src/rcr/PowerSelector.java           火力选择：期望得分最大化（BeepBoop 模型移植）
src/rcr/Knn.java                     KNN 样本库（枪/冲浪共用，线性扫描）
...
数据导出：枪 `-Drcr.datalog=<csv>`，冲浪 `-Drcr.surfdata=<csv>`，均需 `-DNOSECURITY=true`（仅本地 datagen）。
