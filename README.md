...
src/pc/Wavelet.java                  主力机器人：事件路由 / 雷达锁 / 能量簿记 / 健康指标（pc.Wavelet）
src/pc/Surfing.java                  True Surfing 走位（两波精确预测 + KNN 危险密度 + shadows/flattener）
src/pc/KnnGun.java                   KNN(DC) 双枪（通用 + anti-surfer，虚拟枪框架选枪）
src/pc/PowerSelector.java            火力选择：期望得分最大化（BeepBoop 模型移植）
src/pc/Knn.java                      KNN 样本库（枪/冲浪共用，线性扫描）
src/pc/RcMath.java                   几何 / 物理 / 墙壁平滑公用函数
src/pc/Snapshot.java                 双方状态快照（敌波回溯用）
src/pc/GeomTest.java                 精确交点的暴力采样自检（java -cp out\classes;robocode.jar pc.GeomTest）
src/pc/ShadowTest.java               bullet shadow 几何的暴力采样自检（同上，pc.ShadowTest）
...
```powershell
# 编译 + 打包成 pc.Wavelet_dev.jar 部署进 C:\robocode\robots + 无头跑一场 35 回合
...
GUI 观战：打开 Robocode，新建对战选 `pc.Wavelet dev`（开 Paint 可看到敌波圈）。
对战结束后 `C:\robocode\robots\.data\pc\Wavelet.data\stats.txt` 会记录跳过回合数与 KNN 数据量。
...
```powershell
.\scripts\datagen.ps1 -Battles 2 -Rounds 35           # 默认枪+冲浪都采（-Gun / -Surf 可单采）
python ml\train_gun_weights.py                        # 枪嵌入权重
python ml\train_surf_weights.py                       # 冲浪嵌入权重
python ml\eval_per_enemy.py                           # 枪权重按对手分解
```

数据导出：枪 `-Dpc.datalog=<csv>`，冲浪 `-Dpc.surfdata=<csv>`，均需 `-DNOSECURITY=true`（仅本地 datagen）。
需要 Python 3.11 + `pip install torch --index-url https://download.pytorch.org/whl/cpu` + numpy。

## 路线图(详见报告 Recommendations 一节)

- **阶段 0（第 1–2 周）——已完成（2026-07-05）**：True 冲浪 + KNN(DC) 单枪，健康指标（skipped turns / 撞墙 / KNN 数据量）写入 `robots\.data\pc\Wavelet.data\stats.txt`。参考骨架（Diamond / BeepBoop / Saguaro）已克隆到 `reference/`。
...
- **阶段 2.5 走位统计 KNN 化 + 嵌入权重学习——已完成（2026-07-13）**：
  - **替换 3×3 分段 bin**：敌波开火时记下 8 维局面特征 `{bft,|latV|,advV,accel,dirTime,wallF,wallB,power}`；真实命中 / flattener 伪命中写入 `SURF_DATA`（容量 2 万）；评估危险时取 top-50 近邻在 GF 轴铺核 → 与旧 `w.stats` 同口径的 bin 数组（GF0 播种保留）；
